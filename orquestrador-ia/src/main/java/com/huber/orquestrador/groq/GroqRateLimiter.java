...
/**
 * Impede que o app ultrapasse o plano gratuito do Groq: aplica sobre os limites reais do plano a
...
public class GroqRateLimiter {
    ...
    public GroqRateLimiter(GroqLimiteProperties limitesDoPlano, GroqUsoDiarioRepository usoDiarioRepository,
        ...
        log.info("Limites efetivos do Groq ({}% do plano): {} req/min, {} req/dia, {} tokens/min, {} tokens/dia. "
                        + "Uso já registrado hoje: {} req, {} tokens.",
                configuracaoService.getCotaGroq(), limiteRequisicoesPorMinuto(), limiteRequisicoesPorDia(),
    ...
    private int aplicarCota(int limiteDoPlano) {
        double fator = Math.max(0, Math.min(100, configuracaoService.getCotaGroq())) / 100.0;
    ...
    public synchronized void reservarRequisicao() {
        ...
        if (usoAtual.getRequisicoes() >= limiteRequisicoesPorDia() || usoAtual.getTokens() >= limiteTokensPorDia()) {
            throw new LimiteGroqAtingidoException(
                    "Limite diário seguro do Groq atingido (%d req/dia ou %d tokens/dia, %d%% do plano). Tente novamente amanhã."
                            .formatted(limiteRequisicoesPorDia(), limiteTokensPorDia(), configuracaoService.getCotaGroq()));
        }
